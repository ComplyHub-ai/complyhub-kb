...
**Proposed fix:** update `v_tas_progress` to read from `tas_builds.updated_at` (or
`GREATEST(d.updated_at, b.updated_at)`) — requires a migration.

**Decision needed from Brian:** confirm fix approach, then this is ready for a `fix/*` branch.
